...
func createTodo(c *gin.Context) {
	...
	update := fmt.Sprintf("New todo created: %v", newTodo)
	if err := nc.Publish("todo-updates", []byte(update)); err != nil {
		fmt.Println("Failed to publish new todo to NATS:", err)
	}
...
func putTodo(c *gin.Context) {
	...
	update := fmt.Sprintf("Todo updated: %v", todo)
	if err := nc.Publish("todo-updates", []byte(update)); err != nil {
		fmt.Println("Failed to publish update todo to NATS:", err)
	}
